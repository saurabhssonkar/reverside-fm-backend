 ...
 * 
 * Tera existing app.js replace karo is se.
 * Yeh file sab kuch link karta hai:
 * mediasoup + socket.io + redis + recording + REST routes
 */
...
// ── Internal modules ──────────────────────────────────────────────────────
import { createWorkerPool, getWorkers } from './mediasoup/WorkerPool.js';
import { initRedis, getRedisClient } from './Redis/RedisManager.js';
import { registerAllSocketHandlers } from './socket/SocketManager.js';
import { recoverIncompleteUploads } from './recording/RecordingService.js';

// ── REST route files ──────────────────────────────────────────────────────
import roomRoutes from './controller/roomRoutes.js';
import recordingRoutes from './controller/recordingRoutes.js';
...
app.use('/recordings', express.static(mergedDir));

// ── REST Routes ───────────────────────────────────────────────────────────
app.use('/api/rooms', roomRoutes);
app.use('/api/recordings', recordingRoutes);

// Health check — test karo server chal raha hai ya nahi
  ...
  try {
    ...
    res.json({
      ...
      mediasoup: {
        ...
        rtcPorts: `${CONFIG.mediasoup.rtcMinPort}–${CONFIG.mediasoup.rtcMaxPort}`,
      },
      redis: 'connected',
    });
  } catch (err) {
...
const io = new Server(httpsServer, {
  ...
  maxHttpBufferSize: 1e7, // 10MB — needed for recording chunks
});

// ── BOOTSTRAP — Start everything in order ─────────────────────────────────
async function bootstrap() {
  console.log('\n🚀 Starting Riverside Clone Backend...\n');
  ...
  try {
    ...
    // Step 2: Mediasoup worker pool (1 per CPU core)
    await createWorkerPool();
    console.log(`✅ Mediasoup workers ready`);

    // Step 3: Recover any incomplete S3 uploads from previous crash
    await recoverIncompleteUploads();
    console.log('✅ Upload recovery checked');

    // Step 4: Register all socket.io handlers
    registerAllSocketHandlers(io);
    console.log('✅ Socket handlers registered');

    // Step 5: Start listening
    httpsServer.listen(CONFIG.server.port, () => {
      console.log(`\n✅ Server running → https://localhost:${CONFIG.server.port}`);
      console.log(`✅ Health check  → https://localhost:${CONFIG.server.port}/api/health\n`);
    });
